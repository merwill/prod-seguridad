...
return array(
	'controllers' => array(
        'invokables' => array(
           ...
           // 'AuthDoctrine\Validation\PasswordStrength' => 'AuthDoctrine\Validation\PasswordStrengthValidation',
        ),
    ),	
    'router' => array(
        'routes' => array(
			'auth-doctrine' => array(
				'type'    => 'Literal',
				'options' => array(
					'route'    => '/auth-doctrine',  //'/auth-doctrine',
					'defaults' => array(
        ...
        'authentication' => array( // this part is for the Auth adapter from DoctrineModule/Authentication
            'orm_default' => array(
                ...
                'credential_callable' => function(\Application\Entity\Usuario $user, $passwordGiven) { // not only User
					...
					//if ($user->getUsrPassword() == md5($passwordGiven)
					 //$user->getUsrActive() == 1) {
                    if (true){
						return true;
					}
