 ...
 */
class ServicioRestController extends AbstractRestfulController
{
    /**
    ...
    public function get($username)
    {
    	...
    	if (!$userData) {
    		return new JsonModel(array(
    				'result' => false,
    				'mensaje' => array('Usuario no identificadfdfoss111s.')));
    	}


    	$usrIdPersona = $userData[0]['usrIdPersona'] != null ? $userData[0]['usrIdPersona'] : 0;
        ...
        if (!$persona) {
            return new JsonModel(array(
                'result' => false,
                'mensaje' => array('Usuario no registrado.')));
        }
    	...
    	if($perfiles){
    		...
    		foreach ($perfiles as $perfil){
                ...
                $sql = "SELECT r.nombre,r.route
                        FROM recursos r JOIN perfil_recurso pr ON r.id = pr.id_recurso and pr.id_perfil = ".$perfil['id']."
                            JOIN perfil p on p.id = pr.id_perfil
                        WHERE pr.estado = true";

                $recursos = $util->execNativeSql($sql);

                //$recursos

                $lista_permisos[$perfil['sigla']] = $recursos;

                $aux = array('id_perfil'=>$perfil['id'],
                            'nombre_perfil'=>$perfil['nombre'],
                            'sigla_perfil'=>$perfil['sigla'] ,
                            'recursos_perfil'=>$recursos );

                $listaPerfilesPermisos[] = $aux;
    	...
    	/*if ($usuarioPerfilResult[0]["idUnidad"] != $idUnidadOrganizacional) {
    		return new JsonModel(array(
    				'result' => false,
    				'mensaje' => array('Usuario no asignado a Unidad Organizacional, consulte con el Administrador. Gracias.')));
    	}*/
...
/*    	$lista_permisos =array('permissions' => array(
    	...
    	$datosSesionArray = array(
    			...
    			'id_persona'=>$usrIdPersona,
    			'id_usuario'=>$usrId,
    			'lista_perfiles_permisos'=>$listaPerfilesPermisos,
    			...
    			'lista_aplicaciones_reg'=>$aplicacionResult,

    			'url_seguridad'=>"http://seguridad",
    			'id_usuario'=>$usrId,
